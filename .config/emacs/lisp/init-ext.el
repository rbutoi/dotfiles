;;; init-ext.el --- External integrations  -*- lexical-binding: t; -*-

(setopt dired-mouse-drag-files t)

(use-package dired-hide-dotfiles
  :general (:keymaps 'dired-mode-map "." 'dired-hide-dotfiles-mode))
(use-package dired-gitignore
  :hook (dired-load . dired-gitignore-global-mode)
  :general (:keymaps 'dired-mode-map "M-." 'dired-gitignore-global-mode))

(use-package xclip :hook elpaca-after-init) ; {clipboard,mouse} in terminal Emacs
(add-hook 'elpaca-after-init-hook #'xterm-mouse-mode)

(use-package vterm :custom (vterm-always-compile-module t))
(use-package vterm-toggle
  :general ("<f5>" 'vterm-toggle)
  (:keymaps 'vterm-mode-map
            "<f5>" 'vterm-toggle))

(use-package man :ensure nil
  :custom
  (Man-width-max nil)
  (Man-notify-method 'aggressive)
  :general (:keymaps 'Man-mode-map "/" 'isearch-forward-word))

(use-package google-this :hook elpaca-after-init)

(use-package atomic-chrome :config (atomic-chrome-start-server)) ; edit browser textareas in Emacs


(provide 'init-ext)
